...
export default function Inbox() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await messagesApi.getAll({ direction: 'inbound', page, limit, search: search || undefined })
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-2xl font-bold text-gray-800">SMS Ricevuti</h2>
        <span className="text-sm text-gray-500">{total} messaggi</span>
      </div>

      <input
        type="text"
        placeholder="Cerca per mittente o testo..."
        value={search}
